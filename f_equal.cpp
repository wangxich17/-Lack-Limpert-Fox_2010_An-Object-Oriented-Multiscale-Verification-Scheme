...
MatrixXf f_equal(MatrixXcf Zjk, MatrixXcf Zj, MatrixXf Zjk_intensity, MatrixXf Zj_intensity)
{
	...
	int b = Zj_intensity.cols(); // Zjk的每一列存储了一个cells的向量信息，因此矩阵Zj的列数b既是cells的数目
	MatrixXf T_res(1, 8);             // Tres()用来累计truth的第i个cell和forecast的第i个cell的误差
	T_res.setZero();                  //初始化
	int visit[cells];                  //标志是否访问visit        
	memset(visit, 0, sizeof(visit));   //用来标记forecast的cells匹配情况

	for (int i = 0; i < b; i++) {
		float Cx = Zj_intensity(0, i);    //中心坐标Cx
		float Cy = Zj_intensity(1, i);   //中心坐标Cy
		int loc = f_equal_close(Cx, Cy, Zjk_intensity, visit, b);//找出位置为loc的未访问的距离truth的第i个cell的中心(Cx, Cy)最近的forecst的cell 进行match
		cout <<"i="<<i<<" loc="<< loc << endl;
		//循环赋值矩阵t_Zj和t_Zjk（复数形式）,用于后面计算
		MatrixXcf t_Zj(Zj_intensity.rows(), 1);
		for (int j = 0; j < Zj_intensity.rows(); j++) {
			t_Zj(j, 0)._Val[0] = Zj(j, i)._Val[0];
			t_Zj(j, 0)._Val[1] = Zj(j, i)._Val[1];
		}
		MatrixXcf t_Zjk(Zjk_intensity.rows(), 1);
		for (int j = 0; j < Zjk_intensity.rows(); j++) {
